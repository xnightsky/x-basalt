...
> 注意：frontmatter 日期精确等值 `= "2026-06-28"` 通常不命中（YAML 解析为 Date 后以完整 ISO 串存储），请用 `>= "2026-06-28" AND < "2026-06-29"` 替代。详见 [indexing-and-sync.md](indexing.md)。
...
x-basalt 通过 SQLite JOIN 实时计算隐式字段（**不建物化视图**，硬约束）。数据模型细节见 [indexing-and-sync.md](indexing.md)。
...
> 数据模型（`files`/`links`/`tags`/`tasks`/`blocks` 表结构）与链接解析细节见 [indexing-and-sync.md](indexing.md)。
