---
timestamp: 2026-07-30T06:43:07Z
sha256: 75573589ebbdf6de72d778ccdb1453dcf95cb5858e76406fae06154de4fc5508
type: guide
...
>
> 上级索引：[使用指南](README.md) ·
> 相关章节：[命令参考](commands.md) · [索引与同步](indexing.md) · [故障排查](troubleshooting.md)
...
| `pipelines` | `--pipe use=<name>`（run/scan/watch 共用）   | **结构化对象**：命名管道快照（`name → {actions, where, on, paths, debounce, concurrency, onError, onBusy, ifExists, dryRun}`）。**命令行 `--pipe k=v` 与之一一对应**（多词 key 命令行用 kebab-case：`--pipe if-exists` ⟷ `ifExists`）——命令行是规范落地、配置段是加速；非法值在**加载期报错**并定位到 `pipelines.<name>.<key>`。见 [commands.md `run`](commands.md#run--变更编排管道) |
