...
| `pipelines` | `--pipe use=<name>`（run/scan/watch 共用）   | **结构化对象**：命名管道快照（`name → {actions, steps, where, on, paths, debounce, concurrency, onError, onBusy, ifExists, dryRun}`；`actions` 与 `steps` 至少其一，`steps` 存在时优先）。**命令行 `--pipe k=v` 与之一一对应**（多词 key 命令行用 kebab-case：`--pipe if-exists` ⟷ `ifExists`；`--pipe step=<spec>` 可重复 ⟷ `steps` 数组，一元素一算子 spec、不切分）——命令行是规范落地、配置段是加速；非法值在**加载期报错**并定位到 `pipelines.<name>.<key>`。见 [commands.md `run`](commands.md#run--变更编排管道) |
