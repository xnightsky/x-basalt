...
> 章节归属：[使用指南索引 →](README.md) · 同级章节：[installation.md](install.md) · [commands.md](commands.md) · [querying-dql.md](dql.md) · [indexing-and-sync.md](indexing.md) · [configuration.md](config.md) · [ai-and-skills.md](ai-and-skills.md) · [troubleshooting.md](troubleshooting.md)
...
| `#概念。`（标签紧跟全角句号）           | ✓ 但只取 `概念`（句号终止）   |

精确规则：`#` 前**不能是**字母、数字、下划线（Unicode `\p{L}\p{N}_`），由此排除 wikilink 锚点（`[[Note#heading]]`）被误当标签，同时允许 CJK 标点后紧接的标签。**标签体**只取 Unicode 字母/数字/下划线/连字符/斜杠（`[\p{L}\p{N}_/-]+`），遇其余字符即终止。

> [!note] 已知有意差异（documented boundary）
> Obsidian 实际实现会把**紧跟标签的非空白 Unicode 标点**计入标签本身：正文写 `再打一次 #概念。`，Obsidian 得到的标签是 `概念。`，x-basalt 在标点处终止得 `概念`。
>
> 这是**有意不跟**，不是遗漏。该边界在 Obsidian 侧没有权威规格——官方文档所列允许字符与实现不符（且以 emoji / 符号被接受为由的报告被 closed as not planned），实际字符集还在逐版本变动。跟随它会让中文笔记里的标签按句读碎片化（`概念` / `概念。` / `概念，` 各成一个独立标签），并使 `FROM #概念` 这类前缀查询在句末标签上失效（`概念。` 不是 `概念` 的子标签）。
...
| 同名 basename 链接歧义                     | 取索引中首个匹配（近似）        | 多文件同名时可能指向错误文件；路径感知解析详见 [indexing-and-sync.md](indexing.md) |
