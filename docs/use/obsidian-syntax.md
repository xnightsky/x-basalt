...
>
> 这是**有意不跟**，不是遗漏。该边界在 Obsidian 侧没有权威规格——官方文档所列的允许字符与实测行为不符（对照验证实测：Obsidian 收下 `#概念。`，而其文档列出的字符集里没有全角句号），实际字符集还随版本变动。跟随它会让中文笔记里的标签按句读碎片化（`概念` / `概念。` / `概念，` 各成一个独立标签），并使 `FROM #概念` 这类前缀查询在句末标签上失效（`概念。` 不是 `概念` 的子标签）。
