...
tags:
  ...
  - diagnostic
  - x-basalt
timestamp: 2026-07-22T03:39:46Z
sha256: edf90c9944d1c9cf0ddbfecd423aaffa498b88b57d4653fc4fae270ec83e7e32
---
...
2. **契约位置**：新建 `src/issue.ts` 作为**中立叶子模块**（对齐 `src/config.ts` / `src/format.ts` 扁平风格）承载公共契约真相源；`src/links/types.ts` re-export `BasaltDiagnostic` 保后向兼容，并保留 links 专有 `LinkIssueReason` / `TargetIndex` / `LinkFinding` / `CollectedFile`。依赖方向：`links/types → issue`（叶子，无回边）、`lint/index → issue + links.checkVault`，无环。
3. **reason 放宽**：公共 `reason?: string`（机器可读原因）；links 侧仍产 `LinkIssueReason` 字面量（`string` 子集，赋值兼容），为 P3 metadata 规则 reason（如 `required_missing`）留共用空间。
...
- [ ] **Task 1 — 公共契约落 `src/issue.ts` + links re-export**
  - 新建 `src/issue.ts`：`BasaltDiagnostic` / `BasaltDiagnosticSeverity`（`reason?: string`），文件头注释指向 design §6、标注「公共稳定契约」。
  - `src/links/types.ts`：删本地 `BasaltIssue` 定义，改 `export type { BasaltDiagnostic } from "../issue.js"`；保留 `LinkIssueReason` / `TargetIndex` / `LinkFinding` / `CollectedFile`；更新文件头注释。
  - 全 `src/links/*.ts` + `tests/links/*.ts` 把 `BasaltIssue` 换 `BasaltDiagnostic`（类型引用 + 注释）。
  - 新增 `tests/issue.test.ts`：断言契约字段形状（运行期最小断言 + 编译期消费）。
  - Verify：`pnpm run typecheck` + 受影响 `tests/links/*`。
...
- [ ] **Task 4 — 收口**
  - 契约对账：design / 本计划 / TODO / `AGENTS.md` 目录结构段（如需补 `src/issue.ts`、`src/lint/`）。
...
- 落地后：`src/issue.ts` 注释回指 design §6；`AGENTS.md` 目录结构段按需补 `src/issue.ts` / `src/lint/`。
