...
> **2026-07-01 盲区补录（本轮 deep-research 漏项）**：本轮只盘了「函数/lambda/类型」，漏掉了**算子级**最基础的存在性/取反——一元 `!field`、裸字段真值 `WHERE field`（官方 `parse.ts` 的 `negatedField` 一等文法 + `Values.isTruthy()` 是 WHERE 唯一布尔门），x-basalt 当时**完全缺**且 `= null` 存在语义偏离。这是「有多少篇没有 X」这类高频查询的刚需。已由 [`../specs/2026-07-01-dql-truthiness-existence-design.md`](../specs/2026-07-01-dql-truthiness-existence-design.md) **补齐并 resolved**（补充 deep-research `wf_ab18b7c3`：缺键与显式 null 官方不可区分、均 falsy；`compareValue()` 归一 `undefined→null`）。

| 能力 | 高频 | 现状 | 影响 |
|---|---|---|---|
| **一元 `!` / 裸字段真值**（isTruthy 存在性/取反） | ★核心 | ✅ **已补**（2026-07-01，`truthy` 节点 → json_type CASE；`!field`=not(truthy)） | ~~原完全缺、`!index`/裸 `index` 报错~~；已对标官方 `isTruthy`，`= null` 重定位为显式 null 比较 |
