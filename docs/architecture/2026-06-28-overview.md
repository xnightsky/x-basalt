...
|---|---|---|---|---|---|---|
| **cli** | `src/cli.ts` | commander 装配 7 命令组，只做参数装配与输出，不内联业务逻辑 | `x-basalt parse/index/scan/query/skill/meta/watch` | 终端用户 / AI | 四层库 + config + format | 逻辑在各层，CLI 不持有状态 |
...
| **skill** | `src/skill/` | json5 加载 + Fuse.js 模糊召回规范，内置兜底 | `SkillRecall`(recall/list)、`loadSkills`、`SkillDefinition` | cli | fuse.js、json5、skills/*.json5 | 空目录兜底在 loader；外部目录可 shadow 内置；单文件失败跳过 |
...
src/skill/    Skill 召回：json5 加载 + Fuse 模糊匹配 + 兜底
src/utils/    路径与连接键工具
src/cli.ts    commander 入口
skills/       运行时 Skill 数据（SkillRecall 加载）
tests/        Node 原生测试 + fixtures/sample-vault
```

## 11. 现状与边界（截至 2026-06-28）
