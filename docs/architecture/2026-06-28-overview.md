...
tags:
  ...
  - overview
  - reference
timestamp: 2026-06-28T02:54:29Z
sha256: 968143a9de45f480f47176675f83be8df11c0504e27e7b7f8bfe24d24a2c3708
---
# x-basalt 架构总览
...
flowchart TD
    ...
    indexer -. 只读内容 .-> md
    meta -- 原子读改写 --> md
    skill --> skfiles[("skill-data/*.json5")]
...
flowchart LR
    ...
    P --> nodes["ObsidianNode[]<br/>wikilink/tag/callout/task/highlight/blockRef"]
    ...
    IDX --> T4[("tasks")]
    IDX --> T5[("blocks")]
    T1 & T2 & T3 & T4 & T5 --> Q["DataviewEngine.query(dql)"]
...
erDiagram
    files {
        TEXT path PK "相对 Vault 的 POSIX 路径"
...
| **parser** | `src/parser/` | 内容 → `ObsidianNode[]`，纯函数零 I/O | `VaultParser`、`ObsidianNode`、`ParsedFile` | indexer | gray-matter、utils/path | 不碰 fs/DB；产出是 indexer/query 唯一解析输入；代码区域掩码；按结构去重 |
| **indexer** | `src/indexer/` | 唯一写 SQLite 的边界，全量/增量维护五表 | `VaultIndexer`(rebuild/scan/scanIter/update/remove/watch/close)、`createSchema` | cli、watcher 回调 | parser、better-sqlite3 | 不内联 DQL；隐式字段不物化；先删后插事务；单文件失败跳过 |
...
| **skill** | `src/skill/` | json5 加载 + Fuse.js 模糊召回 + 按名取 / 渲染，内置兜底 | `SkillRecall`(list/get/all/recall/resolvedDir)、`loadSkills`、`renderSkill`、`SkillDefinition` | cli | fuse.js、json5、skill-data/*.json5 | 空目录兜底在 loader；外部目录可 shadow 内置；单文件失败跳过 |
...
skill-data/   运行时 Skill 数据（SkillRecall 加载）
...
1. **目录解析** `resolveSkillDir`：显式 `skillPath`（config）> env `OBSIDIAN_SKILL_PATH` > `~/.obsidian-core/skills`（存在时）> 内置 `skill-data/`（随包发布，`../../skill-data` 上溯两级，src 与 dist 一致）。
2. **加载校验** `loadDir`：读目录下全部 `*.json5` → `JSON5.parse` → 最小校验（须有 `name` + `rules` 数组，否则 warn 跳过，单文件失败不中断）→ `SkillDefinition[]`。
3. **兜底补齐**：`ALWAYS_AVAILABLE`（`obsidian-base-spec` / `x-basalt`）缺失时从内置 `skill-data/` 补回，使基础规范与自我说明书永远可召回；外部目录自带同名则不覆盖（可 shadow）。
