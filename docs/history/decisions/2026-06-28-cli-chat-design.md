...
- **典型链路**：`"找讲 CAP 定理的笔记"` → 模型发 `search`/`query` → observe 命中文件 → 模型读取/汇总 → 回答。（注：纯结构化 DQL 查不了正文，"找内容"这步要靠 [检索后端 spec](../../design/semantic-retrieval.md) 的 FTS5；没有它时 chat 仍能做结构化任务，只是"按内容找"会弱。）
...
**"按内容找"依赖**：chat 的核心价值之一是自然语言找笔记，但纯 DQL 查不了正文——这要 [检索后端 spec](../../design/semantic-retrieval.md) 的 FTS5 先落地。故两份文档是组合关系：chat 是前端，FTS5/语义检索是它最重要的一个后端动作。**chat 不依赖语义检索也能做结构化任务，但有了它才完整。**
...
- **何时才值得做**（触发条件，满足再立计划）：
  ...
  2. [检索后端](../../design/semantic-retrieval.md) 的 FTS5 已落地（否则 chat 的"按内容找"先天残缺）；且
