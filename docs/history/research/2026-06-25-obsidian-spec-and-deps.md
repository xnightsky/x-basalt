...
> **[2026-06-26 偏差标注]** 本表所列 `unified` / `remark-parse` / `@flowershow/remark-wiki-link` / `zod` 在 `src/**` 实测 import 次数 = 0（已成死依赖，解析全为手写字符串）。「该买该建」的重新评估见 [`../specs/2026-06-26-deps-build-vs-buy.md`](../decisions/2026-06-26-deps-build-vs-buy.md)；另核实到 `@portaljs/remark-wiki-link@1.2.0`、`remark-obsidian-md@1.1.0`（含 callout/highlight）可作解析层备选。
