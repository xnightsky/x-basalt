...
```typescript
type ObsidianNode =
  | { type: "wikilink"; target: string; alias?: string; heading?: string; blockId?: string; embed: boolean }
...
interface ParsedFile { frontmatter: Record<string, unknown>; nodes: ObsidianNode[]; }

class VaultParser { parse(content: string): ParsedFile; }
```
...
```typescript
interface QueryResult { type: "LIST" | "TABLE"; columns: string[]; rows: Record<string, unknown>[]; }
class DataviewEngine { constructor(dbPath: string); query(dql: string): QueryResult; }
```
