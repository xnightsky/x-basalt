...
新增 **`scan` 子命令** + `VaultIndexer.scan()`：对比文件系统与库内快照，只处理变化文件。变更检测**默认 mtime+size**，`--rehash` 改走内容对比。**不改 schema**（mtime/size/content 均已存）。
...
## 核心算法：`VaultIndexer.scan(opts)`

1. **扫 FS**：`collectMarkdownFiles(vault)` 取当前所有 `.md`；对每个 `stat` 出 `{ relPath, mtime(floored-ms), size }`。
2. **读库快照**：`SELECT path, mtime, size FROM files` → `Map<relPath, {mtime,size}>`。
3. **算差异**（纯函数 `diffScan(fsList, dbMap, mode)`）：
   - **新增** = 在 FS 不在库；
   - **删除** = 在库不在 FS；
   - **改动**：两边都有，且
     - 默认：`mtime ≠` 或 `size ≠`（floored-ms 比较，同机同文件，等价 git stat 快判）；
     - `--rehash`：读当前内容，与库内 `files.content` 不相等 → 改动（绕开 mtime 漏判）；
   - **未变** = 其余 → **跳过，连读都不读**（默认路径的性能收益）。
4. **落库**（非 dry-run）：手动 `BEGIN/COMMIT`（同 `rebuild`，整体失败 `ROLLBACK`）：
   - 新增 + 改动：`buildPayload` → `deleteByPath`（改动）→ `insertPayload`（先删后插，幂等）；
   - 删除：`deleteByPath`。
5. **返回报告** `{ added: string[], modified: string[], deleted: string[], unchanged: number }`。
