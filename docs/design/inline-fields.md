...
description: x-basalt inline fields 设计真相源：Dataview 三形态（整行/方括号/圆括号）提取正则、inline_fields 表 DDL、查询期 frontmatter ∪ inline 字段解析 SQL（COALESCE）、D1–D5 决策（已拍板：取推荐默认）、安全与已知限制；实现切口与验收见同日 plan
tags:
  - design
  - inline-fields
  - dql
...
## 4. 决策 D1–D5（已拍板 2026-07-02：均取推荐默认）

已随本 spec 冻结；实现期不得再改，改需走修订并按 §9 同步真相源。
...
- **[模型] D3 last-wins 使「一篇清单每行一条」场景失真（2026-07-02 实测补注）**：§1 的读书清单示例里 `- [[三体]] (rating:: 5)` 与 `- [[基地]] (rating:: 4)` 同篇两行，整篇 `rating` 只保留最后的 `"4"`——字段是**文件级**的，想每本书各一个分数须每本书一篇笔记。Dataview 原版会聚成列表，对应 backlog「多值列表化」（按 §5 调研定案默认不做）。教程已向使用者示警：[`../guides/tutorial-rating-inline-fields.md`](../use/tutorial-inline-fields.md) 坑 3。
- Backlog 见 §5。
