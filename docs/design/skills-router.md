...
`scan`/`run`/`base`/`lint` 都有 `--json`，唯独 `query` 不接受，写上就撞 `unknown option`。而 `query` 的输出**本来就恒为 JSON**（`emit` 默认 json，它连 `--format` 都没有）。

补上这个 flag，不改变任何输出。理由是**可类推性**：消费者（人与 AI）按「其它命令都有它」推断是完全合理的，让每个调用方各撞一次再退回裸调，成本远高于接受一个无副作用的 flag。
