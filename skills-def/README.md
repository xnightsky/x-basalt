...
本目录是 x-basalt-cli **开发期** AI 召回用 skill 的**唯一源码**。`.claude/skills/` 仅为安装产物（已 gitignore），由 `pnpm run skills:install` 从这里拷贝生成，**不要手改安装产物**。

> 区分：`skills-def/` 是开发侧（AI 写代码时召回规范）；仓库根的 `skills/` 是**产品运行时** SkillRecall 加载的数据，两者互不替代。
