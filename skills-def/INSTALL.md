...
开发侧业务 skill 的安装、生效与编写指南。真相源在本目录 `skills-def/`，安装产物在 `.claude/skills/`（已 gitignore）。
...
脚本 `scripts/install-skills.mjs` 会遍历 `skills-def/` 下所有含 `SKILL.md` 的子目录，拷贝到 `.claude/skills/<name>/`（覆盖旧产物）。纯 Node、跨平台、零第三方依赖。

输出示例：

```
✓ 安装 skill: biz-dql-subset
✓ 安装 skill: biz-obsidian-spec
完成：安装 2 个 skill 到 .claude/skills/
```

## 三、在 Claude Code 中生效

1. 跑 `pnpm run skills:install` 生成 `.claude/skills/`。
...
- **改了不生效**：安装产物是拷贝，改 `skills-def/` 后必须重跑 `pnpm run skills:install`，别手改 `.claude/skills/`。
- **`.claude/skills/` 不入仓**：这是预期（gitignore），每个开发者本地各自 install。
