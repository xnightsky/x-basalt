...
const diagnostic = (over: Partial<BasaltDiagnostic>): BasaltDiagnostic => ({
  file: "docs/a.md",
  line: 1,
  column: 1,
...
test("paths 命中被检查文件 → 忽略", () => {
  ...
  assert.equal(m.ignored(diagnostic({ file: "archive/old.md" })), true);
  assert.equal(m.ignored(diagnostic({ file: "docs/a.md" })), false);
});

test("targets 命中目标字符串 → 忽略", () => {
  const m = compileIgnore({ targets: ["http://*", "https://*"] });
  assert.equal(m.ignored(diagnostic({ target: "https://x.com" })), true);
});

test("rules.<rule> 仅对该 rule 忽略指定 file/target", () => {
  const m = compileIgnore({ rules: { "links/no-broken-link": ["legacy/**"] } });
  assert.equal(m.ignored(diagnostic({ file: "legacy/x.md" })), true);
  assert.equal(m.ignored(diagnostic({ file: "legacy/x.md", rule: "links/other" })), false);
});

test("空配置 → 从不忽略", () => {
  assert.equal(compileIgnore(undefined).ignored(diagnostic({})), false);
});
