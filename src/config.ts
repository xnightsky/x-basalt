...
export function parsePipelines(raw: unknown): Record<string, PipelineConfig> {
  ...
  for (const [name, value] of Object.entries(raw as Record<string, unknown>)) {
    ...
    out[name] = {
      ...
      refreshIndex: toBoolean(p.refreshIndex, at("refreshIndex")),
      ifExists:
 ...
 * 的 `{}` 区分开；显式 baseDir 缺配置时由 {@link loadConfig} 视为空项目配置，不回退 cwd。
 */
 ...
 * 显式提供 `baseDir`（CLI 中即 `X_BASALT_DIR`）时，项目配置只从该目录的 `config.*` 读取；
 * 缺文件视为空项目配置，不回退 cwd。未提供时才从 cwd 向上发现 `.x-basalt/config.*` 或扁平配置。
 * 优先级与 CLI flag（flag 最高）由调用方 `flag ?? config.X` 处理；本函数只做「文件层」合并。
 *
 * 该严格单源语义保证 config 与默认 `index.db` 同属一个基目录；否则 cwd 配置会与
 * `$X_BASALT_DIR/index.db` 静默分家，初始化输出看似成功却留下两套状态来源。
 *
 * @param cwd - 起始目录，默认 process.cwd()
 * @param globalHome - 全局配置所在 home，默认 homedir()（测试可注入隔离）
 * @param baseDir - 可选项目配置基目录；提供后严格替代 cwd 就近发现，无需预判目录是否存在
 *
 * @behavior
 ...
 *
 * @behavior
 * Given 设了 X_BASALT_DIR 但该目录不存在、或存在却无 config.*
 * When 加载
 * Then 项目配置为空且不回退 cwd，仍与全局配置合并
 *
 * @behavior
...
export function loadConfig(
  ...
  // 显式 baseDir 是项目配置来源边界：缺 config.* 也只表示空项目配置，不能回退 cwd，
  // 否则 config 会来自 cwd、默认 DB 却落 baseDir，重新引入两套状态的隐性分家。
  const projectCfg =
    baseDir === undefined ? loadProject(explorer, cwd) : (loadConfigDir(explorer, baseDir) ?? {});
