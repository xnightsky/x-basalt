...
## 🧭 2026-07-22 KB compiler P3 · metadata profile lint（P3a + P3b 均已落地）

metadata 侧 lint，分两阶段均已落地（设计见 [design §8](docs/specs/2026-07-09-kb-compiler-lint-links-design.md)；业界依据已核实、信源见计划）：
...
- **P3b**（✅ 已落地）：`.x-basalt/config` 自定义 `profiles.<name>` + `extends` 继承 + `enums`；合并语义（单父/子覆盖/required 并集/enums 按字段只加不减/环检测/未知父报错/同名覆盖内置）+ `include` glob 收窄；新增 rule `metadata/enum-invalid`。计划：[`docs/plans/2026-07-22-kb-compiler-p3b-config-profile.md`](docs/plans/2026-07-22-kb-compiler-p3b-config-profile.md)。
- **后置（P3b.1+，各自待需求再开）**：`excludes`/减字段、多父数组 `extends`、`tagRules`、`domain.fromPath`、config profile 反哺写侧 `meta apply`、`--fix`（P5）、ci/baseline（P4）、完整 JSON Schema。
