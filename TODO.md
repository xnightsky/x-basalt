...
- [x] **P0..P2b 收口 · code review 修复批次**：view 必填字段静默通过、Windows 盘符大小写误判越界、多根 `.base` 主键缺命名空间、`order`/`sort` 静默丢项、`maxOperations` 非查询总额、解析缓存无界。计划：[`docs/plans/2026-07-27-bases-code-review-fixes.md`](docs/plans/2026-07-27-bases-code-review-fixes.md)（2026-07-27 落地，789 全量测试绿）
- [ ] **🐞 不加引号的 YAML 日期不被识别为日期值**（2026-07-27 写教程时实测发现，**静默错**）：frontmatter `due: 2026-08-10` 经 YAML 解析为日期对象，入索引后序列化成 `"2026-08-10T00:00:00.000Z"`（带毫秒）；`values.ts` 的 `parseDateLike` 严格 ISO 正则不接受小数秒，于是该值留在字符串形态——`due < now()` 之类比较产生 `base/property-type-mismatch`（warning）+ cell null，**查询退出码 0、看起来成功但结果不对**。Obsidian 自己写日期属性就是不加引号的，所以真实 vault 普遍命中。两层修法：①`parseDateLike` 接受可选 `.sss`（小改，但把原本的 date 精度变成 datetime）；②indexer 保留 YAML 原生日期的类型信息（治本，动 schema）。需先定口径再开计划；绕法（加引号 / `.obsidian/types.json` 声明 date）已写入 [guides/querying-bases.md](docs/guides/querying-bases.md) §6。
